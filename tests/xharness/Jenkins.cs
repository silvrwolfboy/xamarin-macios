...
namespace xharness
{
	public class Jenkins
	{
		...
		object xml_report_lock  = new object ();
		// unify all the xml results into one
		public void GenerateXmlReport ()
		{
			lock (xml_report_lock) {
				var rootDir = "";
				var finalPath = Path.Combine (rootDir, "tests-grouped-result.xml");
				if (File.Exists (finalPath))
					File.Delete (finalPath);
				// get all the xml files that can be found in the log directory
				var testFilePattern = "test-*.xml";
				var files = Directory.GetFiles (LogDirectory, testFilePattern, SearchOption.AllDirectories);
				if (files.Length == 0) {
					Console.WriteLine ($"Not files were found with the pattern '{testFilePattern}' for direcotry {LogDirectory}");
				}

				if (files.Length == 1) {
					// we only have a single tests so we simply compy the file to the final destination
					File.Copy (Path.Combine (LogDirectory, files [0]), finalPath, true);
					return;
				} 

				// we have more than one file, we need to diff passes, one to get all the info of results, second to build our own result
				var errors = 0L;
				var inconclusive = 0L;
				var ignored = 0L;
				var invalid = 0L;
				...
				foreach ( var filePath in files) {
					...
					try {
						...
						Console.WriteLine ($"Got {e.Message} for file {filePath}");
						Console.WriteLine ("File will be ignored.");
					}
				} //foreach
				var successString = (errors == 0 && failures == 0)? "True" : "False";
				var errorString = (errors == 0 && failures == 0)? "Success" : "Failure";
				using (var writer = File.CreateText (finalPath)) {
					...
					writer.WriteLine ($"  <test-suite type=\"Assemblies\" name=\"Device Tests\" executed=\"True\" success=\"{ successString }\" result=\"{errorString}\" time=\"{time.Value}\">");
				...
				Console.WriteLine ($"File merge completed to file {finalPath}");
			}
		}
